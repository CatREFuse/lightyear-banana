import { HOST_COMMANDS, PROTOCOL_VERSION, type BridgeError, type HistoryUpsertEntry, type HostCommand, type HostEventName, type PlacementTarget } from '../../../packages/inner-protocol/src/index'
...
type PhotoshopActionNotifications = {
  ...
  removeNotificationListener?: (events: NotificationEvent[], listener: NotificationListener) => Promise<void>
}
...
export class CommandRegistry {
  ...
  destroy() {
    ...
    if (this.notificationAttached && this.notificationAction?.removeNotificationListener) {
      void this.notificationAction.removeNotificationListener(CONTEXT_NOTIFICATION_EVENTS, this.notificationListener).catch(() => undefined)
    }
  ...
  private async placeAsset(assetId: string, target: PlacementTarget) {
    ...
    return { layerName: `UXP 插入 ${image.label}`, target: resolvedTarget }
  }
