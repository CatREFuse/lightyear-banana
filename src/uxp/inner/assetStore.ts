...
type UxpFolder = {
  isFolder?: boolean
  getEntry: (name: string) => Promise<UxpFile | UxpFolder>
...
type UxpStorage = {
  ...
  localFileSystem?: { getDataFolder?: () => Promise<UxpFolder> }
}
...
function getUxpStorage(): UxpStorage {
...
export class AssetStore {
    ...
    await this.mutatePersistent(async () => {
      ...
      if (folder) {
        for (const record of index.values()) {
          let file: UxpFile
          try {
            file = await folder.getEntry(record.fileName) as UxpFile
          } catch {
            continue
          }
        ...
        let indexFile: UxpFile | undefined
        try {
          indexFile = await folder.getEntry(PERSISTENT_INDEX) as UxpFile
        } catch {
          indexFile = undefined
        }
        if (indexFile) {
  ...
  private async getPersistentFolder(create: boolean) {
    const fileSystem = getUxpStorage().localFileSystem
    ...
    try {
      ...
      if (typeof (entry as UxpFolder).getEntry !== 'function') throw new Error('生成记录存储目录无效')
      ...
      try {
        ...
        const entry = await dataFolder.getEntry(PERSISTENT_FOLDER)
        if (typeof (entry as UxpFolder).getEntry !== 'function') throw error
        return entry as UxpFolder
      }
    }
  }
  ...
  private async writePersistentIndex(folder: UxpFolder, index: Map<string, PersistentAssetRecord>) {
    const file = await folder.createFile(PERSISTENT_INDEX, { overwrite: true })
    const format = getUxpStorage().formats?.utf8
  ...
  private async deletePersistentFile(folder: UxpFolder, record: PersistentAssetRecord) {
    try {
      const file = await folder.getEntry(record.fileName) as UxpFile
      await file.delete?.()
    } catch {
  ...
  private async restorePersistentAsset(assetId: string) {
    ...
    try {
      ...
      if (!folder) throw new Error('本地生成记录存储不可用')
      const file = await folder.getEntry(record.fileName) as UxpFile
      const format = getUxpStorage().formats?.binary
