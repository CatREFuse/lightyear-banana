...
function assertIdenticalDirectories(sourceDirectory, embeddedDirectory, sourceFiles, embeddedFiles) {
  if (JSON.stringify(sourceFiles) !== JSON.stringify(embeddedFiles)) {
    throw new Error('The CCX embedded WebUI file set does not match webui/dist.')
  }
  ...
  for (const file of sourceFiles) {
    ...
    if (!sourceBytes.equals(embeddedBytes)) {
      throw new Error(`The CCX embedded WebUI differs from webui/dist at ${file}.`)
    }
  }
}
...
export function verifyEmbeddedInnerWebUiProvenance(options = {}) {
  ...
  const sourceDirectory = options.sourceDirectory ?? path.join(projectRoot, 'webui', 'dist')
  const embeddedDirectory = options.embeddedDirectory ?? path.join(projectRoot, 'dist', 'ccx-host', 'webui')
  const packagePath = options.packagePath ?? path.join(projectRoot, 'webui', 'package.json')
  ...
  const packageJson = readJson(packagePath, 'webui/package.json')
  ...
  const sourceRelease = validateReleaseMetadata(
    readJson(path.join(sourceDirectory, 'release.json'), 'webui/dist/release.json'),
    'webui/dist/release.json',
    expectedVersion,
    verifiedProvenance
  )
  ...
  validateCompatibility(
    readJson(path.join(sourceDirectory, 'compatibility.json'), 'webui/dist/compatibility.json'),
    'webui/dist/compatibility.json',
    expectedVersion
  )
  validateCompatibility(
  ...
  if (sourceContentHash !== sourceRelease.contentHash) {
    throw new Error('webui/dist files do not match release.json contentHash.')
  }
