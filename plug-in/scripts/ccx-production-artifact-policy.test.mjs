...
function artifactFixture(context) {
  ...
  mkdirSync(path.join(root, 'assets', 'nested'), { recursive: true })
  ...
  writeFileSync(path.join(root, 'assets', 'nested', 'host.css'), '.app { display: flex; }\n')
  return root
}
...
test('finds every forbidden marker anywhere in the CCX tree', async (context) => {
  const root = artifactFixture(context)

  forbiddenProductionCcxMarkers.forEach((marker, index) => {
    writeFileSync(
      path.join(root, 'assets', 'nested', `leak-${index}${index === 0 ? '.bin' : '.js'}`),
