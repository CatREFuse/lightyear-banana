...
function fixture(t, { dirty = false } = {}) {
  ...
  json(root, 'webui/package.json', { name: '@mugen/inner-webui', version })
  json(root, 'webui/dist/compatibility.json', compatibility)
  write(root, 'webui/dist/index.html', '<main>Mugen</main>')
  write(root, 'webui/dist/assets/index.js', 'console.log("Mugen")')
  write(root, 'webui/dist/assets/index.css', ':root{color-scheme:dark}')
  ...
  json(root, 'webui/dist/release.json', release)
...
test('rejects metadata whose dirty state does not match the actual worktree', (t) => {
  ...
  json(value.root, 'webui/dist/release.json', mismatchedRelease)
...
test('rejects source or embedded metadata from a stale commit', (t) => {
  ...
  json(sourceFixture.root, 'webui/dist/release.json', {
