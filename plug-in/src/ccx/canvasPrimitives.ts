...
import type { CapturedCanvasImage, PixelBounds } from '../../../packages/mugen-core/src/types/canvas'

export type { CapturedCanvasImage, PixelBounds } from '../../../packages/mugen-core/src/types/canvas'
...
type AdobeUxpRuntime = {
  storage?: {
    formats?: {
      binary?: unknown
    }
    localFileSystem?: {
      createSessionToken?: (file: AdobeUxpFile) => string
      getTemporaryFolder?: () => Promise<AdobeUxpFolder>
    }
  }
}
...
function getAdobeUxpRuntime(): AdobeUxpRuntime {
  const hostRequire = getHostRequire()
  if (!hostRequire) {
    throw new Error('Photoshop UXP runtime is unavailable.')
  }

  return hostRequire('uxp') as AdobeUxpRuntime
}

async function emitTrace(
  trace: CcxDiagnosticTrace | undefined,
  operation: string,
...
async function createTemporaryPreviewFile(image: CapturedCanvasImage) {
  ...
  const localFileSystem = adobeUxpRuntime.storage?.localFileSystem
  ...
  const binaryFormat = adobeUxpRuntime.storage?.formats?.binary
...
async function deleteTemporaryFile(file: AdobeUxpFile) {
  try {
    await file.delete?.()
  } catch {
...
async function getSelectionPixels(trace?: CcxDiagnosticTrace) {
  const photoshop = getPhotoshop()
...
async function getVisibleCompositePixels(bounds?: PixelBounds, trace?: CcxDiagnosticTrace) {
  const photoshop = getPhotoshop()
  const document = photoshop.app.activeDocument
...
async function getMergedVisiblePixels(bounds: PixelBounds, trace?: CcxDiagnosticTrace) {
  const photoshop = getPhotoshop()
...
async function getMergedVisibleCompositePixels(bounds?: PixelBounds, trace?: CcxDiagnosticTrace) {
  const photoshop = getPhotoshop()
  const sourceDocument = photoshop.app.activeDocument
...
export async function captureVisibleComposite(trace?: CcxDiagnosticTrace): Promise<CapturedCanvasImage> {
...
export async function captureSelectionComposite(trace?: CcxDiagnosticTrace): Promise<CapturedCanvasImage> {
...
export async function readSelectionBounds(trace?: CcxDiagnosticTrace) {
...
async function placeTemporaryImageFile(
  image: CapturedCanvasImage,
  temporaryFile: { file: AdobeUxpFile; token: string },
  ...
  await photoshop.core.executeAsModal(
    async () => {
      ...
      try {
        layer.name = `Mugen 插入 ${image.label}`
      } catch {
...
async function putCapturedImagePixels(image: CapturedCanvasImage, target: NormalizedInsertTarget, expectedDocumentId?: string) {
  ...
  await photoshop.core.executeAsModal(
    async () => {
      assertActiveDocument(photoshop, expectedDocumentId)
      const layer = await createPixelLayer(`Mugen 插入 ${image.label}`)
...
export async function insertPreviewImage(
  ...
  let temporaryFile: { file: AdobeUxpFile; token: string } | null = null
