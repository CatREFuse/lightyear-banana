...
import { isDisallowedProductionHostname, resolveReleaseUrl } from '../utils/production-origin-policy.mjs'
import { resolveCcxMugenEnvironment } from './scripts/ccx-environment-policy.mjs'

const projectRoot = fileURLToPath(new URL('.', import.meta.url))
const ccxHostOutDir = path.resolve(projectRoot, '..', 'dist', 'ccx-host')
...
function ccxPostBuildPlugin(innerWebUiUrl: URL): Plugin {
  return {
    name: 'ccx-post-build',
    closeBundle() {
      const panelPath = path.join(ccxHostOutDir, 'ccx-panel.html')
      ...
      mkdirSync(ccxHostOutDir, { recursive: true })
...
export default defineConfig(({ mode }) => {
  ...
  const mugenEnvironment = resolveCcxMugenEnvironment(mode, {
  ...
  return {
    ...
    build: {
      ...
      outDir: ccxHostOutDir,
      emptyOutDir: true,
      sourcemap: mugenEnvironment !== 'production',
      rollupOptions: {
        input: fileURLToPath(new URL('./ccx-panel.html', import.meta.url)),
        output: {
          format: 'iife'
        }
      }
