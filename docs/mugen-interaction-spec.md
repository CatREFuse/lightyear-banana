...
Mugen 的核心体验是把 Photoshop 画布变成生图工作流的上下文。用户从当前文档采集参考图，输入提示词，选择模型参数，生成图片，再把结果送回 Photoshop。
...
- 顶部栏标题 `Mugen v0.1`。
- 顶部栏状态为当前 runtime。
- 结果流空状态显示 `READY` 和 `输入提示词，或从 Photoshop 添加参考图`。
...
| 选区 | 抓取当前选区内所有可见图层的合成图和 bounds | 使用 mock 图 |
...
| 类型 | 文案 |
| --- | --- |
| 页面标题 | `Mugen v0.1`、`设置`、`配置详情` |
| 空状态 | `READY`、`输入提示词，或从 Photoshop 添加参考图` |
| 输入占位 | `输入提示词，或输入 / 调用预设` |
