...
- CCX 是 ZIP 容器，归档根目录直接包含 `manifest.json`、`ccx-panel.html`、`assets/`、`icons/` 与 `webui/`，不能再套一层父目录，也不写独立目录条目。
...
- 正式打包要求干净工作树。内嵌 WebUI 的两份 `release.json`、CCX 发布元数据和归档内容都绑定同一个 Git HEAD。
