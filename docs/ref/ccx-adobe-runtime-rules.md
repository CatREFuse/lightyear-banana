...
- 改 Host 壳或 WebUI 后必须运行 `npm run build:ccx`。
- 改 manifest、权限、entrypoint、SWC 开关后必须运行 `npm run verify:ccx`，并在 Adobe UXP Developer Tools 中 `Unload` / `Load`。
...
- `npm run verify:ccx` 必须增加或保留 SWC 相关静态校验。
