...
const port = Number(process.env.MUGEN_MOCK_IMAGE_API_PORT ?? 38322)
const host = process.env.MUGEN_MOCK_IMAGE_API_HOST ?? '127.0.0.1'
const __dirname = path.dirname(fileURLToPath(import.meta.url))
const projectRoot = path.resolve(__dirname, '..')
const fixtureDir = path.join(projectRoot, 'webui', 'public', 'mock-images')
const fixtureFiles = Array.from({ length: 20 }, (_, index) => `cats/cat-${String(index + 1).padStart(2, '0')}.jpg`)
const profile = readOption('--profile') ?? process.env.MUGEN_MOCK_IMAGE_API_PROFILE ?? 'all'
const fixedFixtureFile = readOption('--fixture') ?? process.env.MUGEN_MOCK_IMAGE_API_FIXED_FIXTURE ?? ''
const generationDelayMinMs = Number(readOption('--delay-min-ms') ?? process.env.MUGEN_MOCK_IMAGE_API_DELAY_MIN_MS ?? 3000)
const generationDelayMaxMs = Number(readOption('--delay-max-ms') ?? process.env.MUGEN_MOCK_IMAGE_API_DELAY_MAX_MS ?? 5000)
...
function createOpenAiResponse(requestUrl, count) {
  ...
  return {
    ...
    data: Array.from({ length: count }, (_, index) => ({
      ...
      revised_prompt: 'Mock image generated by Mugen.'
    }))
  }
}
...
function sendManual(response) {
  sendJson(response, 200, {
    server: 'Mugen Image API Mock Server',
    port,
    profile,
...
server.listen(port, host, () => {
  console.log(`Mugen Image API Mock Server: http://${host}:${port}`)
