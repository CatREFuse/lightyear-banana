...
function compileRegressionSources(outDir) {
  ...
  execFileSync(process.execPath, [readTscBin(),
    ...
    'packages/mugen-core/src/utils/imagePixels.ts',
...
async function testRemoteImageDimensionsAndPreviewStyle(requireFromBuild, outDir) {
  ...
  try {
    const imagePixels = requireFromBuild(join(outDir, 'packages', 'mugen-core', 'src', 'utils', 'imagePixels.js'))
  ...
  const messageThread = await readFile(new URL('../webui/src/components/mugen/MessageThread.vue', import.meta.url), 'utf8')
...
async function testNothingThemePreservesGeometry() {
  const themeCss = await readFile(new URL('../webui/src/styles/nothing-theme.css', import.meta.url), 'utf8')
  const mugenPanel = await readFile(new URL('../webui/src/components/mugen/MugenPanel.vue', import.meta.url), 'utf8')
...
async function main() {
  ...
  try {
    ...
    const imageApi = requireFromBuild(join(sourceOutDir, 'packages', 'mugen-core', 'src', 'services', 'imageApiClient.js'))
    const providerCapabilities = requireFromBuild(join(sourceOutDir, 'packages', 'mugen-core', 'src', 'data', 'providerCapabilities.js'))
    const providerRegistry = requireFromBuild(join(sourceOutDir, 'packages', 'mugen-core', 'src', 'providers', 'registry.js'))
    const canvasPrimitives = requireFromBuild(join(sourceOutDir, 'plug-in', 'src', 'ccx', 'canvasPrimitives.js'))
