...
- Electron Main 为每次 `/uxp/*` 请求写入一条完成记录，包含方法、路径、状态码、耗时和关联命令，不写 token 和业务载荷。
- UXP 侧把无法送达 Electron 的连接失败保存在有上限的内存队列中，重连成功后补传。
- 设置页可以分别导出完整诊断日志和 CRX 连接日志；CRX 日志包含 `crx`、`bridge`、`photoshop` 三类关联记录。
- 两类日志都使用最近 24 小时保留窗口，并在导出前再次清理过期记录。
