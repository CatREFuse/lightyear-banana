...
| Photoshop CCX / UXP Host `1.0.0` | 活动 | 插件 ID `com.tanshow.mugen`；继续提供画布、置入、SecureStorage 和原生权限 |
...
| Standalone UXP 插件原型 | 已归档 | 不再承载产品功能；不等于废弃当前 CCX Host |
...
- Photoshop 文档修改仍由 UXP Host 在 `core.executeAsModal()` 中执行。
...
- 新增画布能力先进入 `src/uxp/canvasPrimitives.ts`，再由 `src/uxp/canvasPrimitiveService.ts` 暴露。
...
| `src/uxp/main.ts` | CCX panel、会话和 Host 注册 |
| `src/uxp/inner/` | CCX 资产、历史、Provider、存储与确认能力 |
| `src/uxp/canvasPrimitiveService.ts` | 画布业务服务层 |
| `src/uxp/canvasPrimitives.ts` | Photoshop imaging、选区、图层与写入原语 |
...
截至 2026-08-11，旧基线曾通过 `inner-host/v1` 协议测试、Inner WebUI 0.1 单元与构建测试、部分 Playwright E2E、UXP Host 单元测试、CCX 静态打包和公网 WebUI 校验。`dist/mugen-1.0.0.ccx` 也曾被生成并校验。真实 Photoshop 中的旧 Inner WebUI 完整闭环尚未形成发布证据。
