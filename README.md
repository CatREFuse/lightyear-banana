# Lightyear Banana

在 Photoshop 里连接 AI 生图模型。你可以把当前画布、选区和图层作为参考图，生成新图片后直接置入 Photoshop 文档。

![Lightyear Banana 工作台](docs/images/lightyear-workspace.png)

## 安装

Lightyear Banana 由桌面本体和 Photoshop CCX 插件一起工作。桌面本体负责模型配置、生成任务、结果流和窗口部署；CCX 插件负责连接 Photoshop，读取画布、选区和图层，并把结果写回文档。

### 下载索引

当前版本：`0.3.6`

| 内容 | 用途 | 下载 |
| --- | --- | --- |
| macOS 桌面本体 | 运行 Lightyear App | [`lightyear-banana-0.3.6-mac.zip`](https://github.com/CatREFuse/lightyear-banana/releases/download/v0.3.6/lightyear-banana-0.3.6-mac.zip) |
| Windows 桌面本体 | 运行 Lightyear App | [`lightyear-banana-0.3.6-win.zip`](https://github.com/CatREFuse/lightyear-banana/releases/download/v0.3.6/lightyear-banana-0.3.6-win.zip) |
| Photoshop CCX 插件 | 安装 Photoshop 面板 | [`lightyear-banana-0.3.6.ccx`](https://github.com/CatREFuse/lightyear-banana/releases/download/v0.3.6/lightyear-banana-0.3.6.ccx) |
| Codex Image Server Skill | 把本机 Codex 生图能力改造成 HTTP image server | [`skills/codex-image-server/`](https://github.com/CatREFuse/lightyear-banana/tree/v0.3.6/skills/codex-image-server) |
| 仓库源码 | 包含桌面端、插件和 Codex Image Server Skill | [`Source code zip`](https://github.com/CatREFuse/lightyear-banana/archive/refs/tags/v0.3.6.zip) |

同一版本需要下载 2 个运行文件：

- macOS 用户：`lightyear-banana-0.3.6-mac.zip` 和 `lightyear-banana-0.3.6.ccx`
- Windows 用户：`lightyear-banana-0.3.6-win.zip` 和 `lightyear-banana-0.3.6.ccx`

### 1. 安装桌面本体

macOS：

1. 解压 `lightyear-banana-0.3.6-mac.zip`。
2. 把 `Lightyear Banana.app` 拖到“应用程序”文件夹。
3. 打开 `Lightyear Banana.app`。

Windows：

1. 解压 `lightyear-banana-0.3.6-win.zip`。
2. 打开解压后的文件夹。
3. 运行 `Lightyear Banana.exe`。

启动后会打开 Lightyear App。本地工作台默认运行在 `127.0.0.1:38321`，桌面窗口会自动连接这个本地服务。

### 2. 安装 Photoshop CCX 插件

推荐使用 Adobe Creative Cloud 桌面端安装：

1. 打开 Adobe Creative Cloud 桌面端。
2. 把 `lightyear-banana-0.3.6.ccx` 拖进 Creative Cloud 窗口。
3. 按照弹窗完成安装。
4. 重启 Photoshop，或在 Photoshop 中重新打开插件菜单。

开发调试时也可以用 UXP Developer Tools 加载：

1. 运行 `npm run verify:uxp`。
2. 在 UXP Developer Tools 中选择 `dist/ps-uxp/manifest.json`。
3. 点击 Load。
4. 改过 `plugin/manifest.json` 后，先 Unload，再 Load。

## 打开面板

1. 打开 Lightyear App。
2. 打开 Photoshop 2026。
3. 打开或新建一个 Photoshop 文档。
4. 在 Photoshop 菜单中选择“增效工具 > Lightyear Banana”。
5. 面板顶部显示“Photoshop 已连接”后即可使用。

如果顶部显示“Photoshop 未连接”，先确认 Lightyear App 已打开，再重新打开 Photoshop 面板。

## 配置模型

点击右上角设置按钮进入配置页。

![设置页](docs/images/lightyear-settings.png)

发行版不会内置模型配置、Base URL 或 API Key。点击“新建配置”添加自己的模型。云端模型需要填写对应服务的 API Key。Codex Image Server 需要填写 Base URL；如果服务端启用了 `CODEX_IMAGE_SERVER_API_KEY`，再填写同一个 API Key。

自定义模型支持 OPENAI Images、OPENAI Chat 和 Gemini 三种 API 格式。New API 这类网关使用 Gemini 原生接口时，供应商选“自定义模型”，API 格式选“Gemini”，Base URL 填网关域名、`/v1`、`/v1beta` 或 `/v1beta/models` 都可以。当前已验证 `nanobanana2` 和 `nanobananapro`。

选择 Codex Image Server 供应商后，设置页会显示改造说明入口，用于把本机 Codex 的生图能力暴露成可调用的本地服务。

保存配置后，回到工作台，在底部“模型”菜单中选择要使用的配置。

## Codex Image Server Skill

本仓库提供了一个可复用 skill，位置在 `skills/codex-image-server/`。它用于把 Codex 的 `image_generation` 能力封装成本地 HTTP image server，供 Photoshop 插件、设计工具或内部图像工作流调用。

目录内容：

- `SKILL.md`：agent 使用入口。
- `agents/openai.yaml`：Codex 技能列表展示信息。
- `references/http-contract.md`：本地 HTTP API 契约。
- `scripts/smoke-test.mjs`：本地服务冒烟测试。
- `templates/codex-image-server.js`：可复用 Node 服务模板。

这个 skill 会指导 agent 完成这些工作：

- 检查目标 Codex 安装或源码形态。
- 添加本地 HTTP 服务，提供 `/healthz`、`/v1/capabilities`、`/v1/images/generate` 和 `/v1/images/:id/file`。
- 支持 `gpt-image-2` 自定义分辨率，最长边 3840px，宽高为 16 的倍数，比例不超过 3:1。
- 单次请求最多生成 4 张图。
- 通过 `AbortSignal` 支持取消，并在取消后清理 `codex exec` 进程组。
- 使用原始参考图文件作为 Codex image input。
- 删除占位图、假数据和本地假服务路径。

skill 完成后的返回内容会优先给出 Lightyear Banana 配置指导。默认填写：

- Provider：`Codex Image Server`
- 配置名称：`Codex Image Server`
- Base URL：`http://127.0.0.1:17341`
- 端口：`17341`
- API Key：留空
- 测试入口：设置页点击“测试 API”
- 生成接口：`POST http://127.0.0.1:17341/v1/images/generate`
- 能力接口：`GET http://127.0.0.1:17341/v1/capabilities`

如果启动服务时设置了 `CODEX_IMAGE_SERVER_HOST` 或 `CODEX_IMAGE_SERVER_PORT`，以实际返回的 Base URL 和端口为准。

只下载这个 skill：

```bash
git clone --filter=blob:none --sparse https://github.com/CatREFuse/lightyear-banana.git
cd lightyear-banana
git checkout v0.3.6
git sparse-checkout set skills/codex-image-server
```

安装到本机 Codex：

```bash
mkdir -p ~/.codex/skills
rm -rf ~/.codex/skills/codex-image-server
cp -R skills/codex-image-server ~/.codex/skills/codex-image-server
```

也可以从 [`Source code zip`](https://github.com/CatREFuse/lightyear-banana/archive/refs/tags/v0.3.6.zip) 解压后复制 `skills/codex-image-server/`。

发布到 ClawHub：

```bash
clawhub publish skills/codex-image-server \
  --slug codex-image-server \
  --name "Codex Image Server" \
  --version 0.3.6 \
  --tags codex,image-generation,image-server,photoshop
```

验证本地 image server：

```bash
node skills/codex-image-server/scripts/smoke-test.mjs http://127.0.0.1:17341
```

## 生成图片

1. 在 Photoshop 文档中准备好画布、选区或图层。
2. 在 Lightyear Banana 面板底部点击“添加参考”。
3. 选择“可见图层”“选区”“当前选中图层”“上传文件”或“剪贴板”。
4. 输入提示词。
5. 选择模型、尺寸、质量、数量和比例。
6. 点击“发送”。
7. 等待结果出现在对话流中。

![结果流](docs/images/lightyear-result-flow.png)

结果图下方有 3 个常用动作：

- “置入”：把结果放回 Photoshop，可选择全画布、当前选区或参考图选区。
- “超分”：把这张结果图放入下一轮，并自动填入高分辨率参数。
- “添加参考”：把这张结果图加入当前参考图列表。

## 窗口部署

Lightyear App 顶部有窗口部署按钮，可以把工作台固定在屏幕左侧或右侧，并尝试同步调整 Photoshop 窗口。

macOS 第一次使用窗口部署时，可能需要在系统设置中允许以下权限：

- 辅助功能
- 自动化
- 屏幕录制

设置页顶部提供了对应入口。授权后重新点击窗口部署即可。

## 系统要求

- macOS 14 及以上，或 Windows 10 及以上
- Photoshop 2026，版本 27.3.0 及以上
- Adobe Creative Cloud 桌面端，或 UXP Developer Tools
- 本地端口 `38321` 用于 Lightyear App 和 Photoshop 插件通信

## 常见问题

### Lightyear App 已打开，但 Photoshop 仍显示未连接

先关闭 Photoshop 面板，再从“增效工具 > Lightyear Banana”重新打开。仍未连接时，确认桌面本体没有被系统防火墙拦截，并检查本地端口 `38321` 是否被占用。

### 设置页里显示 API 不可用

云端配置先检查 API Key 是否填写完整。Codex Image Server 和自定义模型还要确认 Base URL 可以访问。

### CCX 安装后菜单里没有 Lightyear Banana

重启 Photoshop。仍未出现时，重新把 `.ccx` 拖入 Adobe Creative Cloud 安装，或用 UXP Developer Tools 加载 `dist/ps-uxp/manifest.json` 检查插件是否能启动。

### 置入 Photoshop 失败

确认当前 Photoshop 文档仍然打开。选择“当前选区”置入时，需要先在 Photoshop 里建立有效选区。

## 开发命令

```bash
npm install
npm run dev
npm run dev:electron
npm run build:uxp
npm run verify:uxp
npm run package:uxp
npm run package:electron:mac
npm run package:electron:win
```

常用产物：

- `dist/lightyear-banana-0.3.6.ccx`
- `dist/lightyear-banana-0.3.6-mac.zip`
- `dist/lightyear-banana-0.3.6-win.zip`
- `dist/release-0.3.6/`

完整发行流程见 `docs/ops-manual.md`。每次发行除了 GitHub Release，还需要同步维护官网首页下载链接和 `https://cake.catrefuse.com/releases/latest.json`。

修改 Vue、TypeScript、CSS 后运行 `npm run build:uxp`。完成 UXP 相关改动后至少运行 `npm run verify:uxp`。修改 manifest、entrypoint、icon 或权限后，在 UXP Developer Tools 中执行 Unload，再重新 Load。
