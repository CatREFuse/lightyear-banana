...
![Mugen 工作台](docs/images/mugen-workspace.png)

## 安装

Mugen 由桌面本体和 Photoshop CCX 插件一起工作。桌面本体负责模型配置、生成任务、结果流和窗口部署；CCX 插件负责连接 Photoshop，读取画布、选区和图层，并把结果写回文档。

同一版本需要下载 2 个文件：

- macOS 用户：`mugen-0.3.19-mac.zip` 和 `mugen-1.0.0.ccx`
- Windows 用户：`mugen-0.3.19-win.zip` 和 `mugen-1.0.0.ccx`

### 1. 安装桌面本体

macOS：

1. 解压 `mugen-0.3.19-mac.zip`。
2. 把 `Mugen.app` 拖到“应用程序”文件夹。
3. 打开 `Mugen.app`。

Windows：

1. 解压 `mugen-0.3.19-win.zip`。
2. 打开解压后的文件夹。
3. 运行 `Mugen.exe`。

启动后会打开 Mugen App。本地工作台默认运行在 `127.0.0.1:38321`，桌面窗口会自动连接这个本地服务。
...
2. 把 `mugen-1.0.0.ccx` 拖进 Creative Cloud 窗口。
3. 按照弹窗完成安装。
...
4. 在 Photoshop 菜单中选择“增效工具 > Mugen”。
5. 面板顶部显示“Photoshop 已连接”后即可使用。

如果顶部显示“Photoshop 未连接”，先确认 Mugen App 已打开，再重新打开 Photoshop 面板。

## 配置模型

点击右上角设置按钮进入配置页。

![设置页](docs/images/mugen-settings.png)

你可以直接编辑默认配置，也可以点击“新建配置”添加自己的模型。每个配置需要选择供应商、模型，并填入对应服务的 API Key。自定义 OpenAI 兼容模型还需要填写 Base URL。
...
![结果流](docs/images/mugen-result-flow.png)
...
Mugen App 顶部有窗口部署按钮，可以把工作台固定在屏幕左侧或右侧，并尝试同步调整 Photoshop 窗口。
...
- 本地端口 `38321` 用于 Mugen App 和 Photoshop 插件通信

## 常见问题

### Mugen App 已打开，但 Photoshop 仍显示未连接

先关闭 Photoshop 面板，再从“增效工具 > Mugen”重新打开。仍未连接时，确认桌面本体没有被系统防火墙拦截，并检查本地端口 `38321` 是否被占用。

### 设置页里显示 API 不可用

检查 API Key 是否填写完整。使用自定义 OpenAI 兼容模型时，还要确认 Base URL 可以访问。
...
- `dist/mugen-1.0.0.ccx`
- `dist/mugen-0.3.19-mac.zip`
- `dist/mugen-0.3.19-win.zip`
- `dist/release-0.3.19/`
