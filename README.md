...
没有真实 API Key 时，可以先用本地 Mock Server 跑通流程。
...
2. 打开设置页。
3. 开启 Mock Server。
4. 保持本地地址为 `http://127.0.0.1:38322`。
5. 使用 `mock-good` 或页面中显示的其他 mock key 测试配置。

![Mock Server](docs/images/lightyear-mock-server.png)

Mock Server 会返回本地测试图片，适合验证发送、结果流、超分、参考图和置入入口。
...
检查 API Key 是否填写完整。使用自定义 OpenAI 兼容模型时，还要确认 Base URL 可以访问。开启 Mock Server 后，可以先用 `mock-good` 验证配置保存和请求流程。
