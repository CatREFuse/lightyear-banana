...
test('creates a complete CCX release metadata record', () => {
  assert.deepEqual(createUxpReleaseMetadata(validInput), {
    schemaVersion: 1,
    ...validInput
  })
})
...
test('requires a normalized credential-free release URL', () => {
  for (const releaseUrl of [
    ...
    assert.throws(() => createUxpReleaseMetadata({ ...validInput, releaseUrl }))
  }
})

test('requires clean source provenance', () => {
  assert.throws(
    () => createUxpReleaseMetadata({ ...validInput, dirty: true }),
    /clean Git worktree/
  )
})
