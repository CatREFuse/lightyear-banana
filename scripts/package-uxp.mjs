...
const releaseMetadataPath = path.join(projectRoot, 'dist', 'uxp-release.json')
...
try {
  ...
  if (!existsSync(temporaryArchivePath)) {
    throw new Error(`Temporary UXP archive was not created: ${temporaryArchivePath}`)
  }
  ...
  if (manifestIndex < 0 || !normalizedEntries.some((entry) => /^assets\/.+\.js$/.test(entry))) {
    throw new Error('UXP archive must contain manifest.json and a bundled JavaScript asset.')
  }
  ...
  if (archivedManifest.id !== builtManifest.id || archivedManifest.version !== builtManifest.version) {
    throw new Error('Archived UXP manifest does not match the verified build.')
  }
...
console.log(`UXP archive checksum: ${archiveChecksumPath}`)
console.log(`UXP release metadata: ${releaseMetadataPath}`)
